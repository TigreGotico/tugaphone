...
class DialectInventory:
    ...
    # =========================================================================
    # BASE PHONEMIZATION SOURCE (B6 stage-2)
    # =========================================================================
    # When True, the out-of-lexicon *base* IPA of a word is generated by the
    # shared orthography2ipa pronunciation lattice (the beam) via
    # :func:`tugaphone.lattice_adapter.lattice_base_ipa`, replacing the private
    # grapheme→phoneme character cascade. The lexicon (IRREGULAR_WORDS),
    # homograph table and the regional accent primitives compose exactly as
    # before, on top of whichever base is produced.
    #
    # Off for every shipped dialect: the current o2i pt specs still have base
    # gaps that would regress the research-backed gold if made the default
    # (see docs/lattice-base-migration.md — coda nasalization absent,
    # <ou> over-monophthongised, pt-BR/AO final unstressed /a/ over-reduced).
    # Flip per dialect as each spec gap closes upstream; the mechanism is
    # otherwise complete and exercised by the test suite.
    USE_O2I_LATTICE_BASE: bool = False
