...
def palatal_affrication_ch(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Realize grapheme <ch> as the voiceless postalveolar affricate /tʃ/ in the phoneme string.
...
def epenthetic_j_before_palatal(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Insert an epenthetic glide `j` after a vowel when it precedes palatal consonants.
    
    This transforms sequences where a vowel is immediately followed by a palatal consonant (ʎ, ɲ, ʃ; optionally tʃ) by inserting `j` between the vowel and the palatal. Existing `j` is preserved (no duplicate insertion).
    
    Returns:
        The phonemes string with `j` inserted before palatal consonants where applicable.
    """
...
def nasal_diphthongization_e(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Diphthongizes nasal /ẽ/ to [eĩ] when it is followed by a consonant.
    
    This transforms occurrences of the nasal vowel `ẽ` into the rising diphthong `eĩ` only when the following segment is a consonant (not a vowel or palatal nasal), leaving other contexts unchanged.
    
    Returns:
        str: The phoneme string with applicable /ẽ/ → /eĩ/ substitutions applied.
    """
...
def nasal_vowel_raising(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Raise the low central nasal vowel /ɐ̃/ to a more open front nasal [ã].
    
    Replaces occurrences of the nasal vowel ɐ̃ with ã to model northern Portuguese nasal-vowel raising (e.g., 'mãe' /mˈɐ̃j/ → /mˈãj/).
    
    Returns:
    	The transformed phoneme string with ɐ̃ replaced by ã.
    """
    # /ɐ̃/ → [ã]
...
def intervocalic_s_voicing(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Voices /s/ to /z/ when it occurs between vowels or word-finally after a vowel.
    
    Applies intervocalic and word-final voicing of /s/ (e.g., between V__V and V__#) to model Northern Portuguese varieties.
    
    Returns:
        The phonemes string with applicable occurrences of `s` replaced by `z`.
    """
...
def initial_z_devoicing(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Devoice an initial /z/ to /s/ when it occurs immediately before a vowel (Transmontano pattern).
    
    Returns:
        The input `phonemes` string with a leading `z` replaced by `s` if it is followed by a vowel, otherwise the original `phonemes`.
    """
    vowels = "aeiouɐɛɔɨẽõɐ̃"
...
def final_nasal_denasalization(word: str, phonemes: str, postag: str = "NOUN") -> str:
    """
    Denasalizes specific final nasal vowels commonly realized in Transmontano speech.
    
    Replaces the nasal vowel endings used in -agem and similar final syllables with their oral counterparts:
    - `ʒẽ` → `ʒe`
    - `ʒɐ̃` → `ʒɐ`
    - `ʒõ` → `ʒo`
    
    Returns:
        The input `phonemes` string with the above final-nasal substitutions applied.
    """
